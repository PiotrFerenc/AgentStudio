...
**Only an `end` node's `OutputTemplate` produces visible output — no other node emits live.** `prompt` and `message` used to write straight to the output channel as they ran (token-by-token for `prompt`, so a live chat showed the LLM "typing"); both now only write to their `ResultVariable` (`message` gained one for exactly this reason — it had no other way to produce output once its own emit was removed). A path through the graph that reaches an `end` node with a blank `OutputTemplate` produces **no visible output at all**, even if every node along the way set variables — this is a deliberate, explicit tradeoff (chosen over keeping `prompt` streaming) to guarantee a run's final reply is always exactly and only what `OutputTemplate` says, never a concatenation of whatever ran along the way. The concrete bug this fixed: a template whose `end` node's `OutputTemplate` referenced `{variables.llmResult}` right after a `prompt` node used to show the LLM's reply **twice** (once from `prompt`'s live emit, once from `end`'s template re-stating the same variable) — impossible now that `prompt` never emits. Every `AgentTemplateCatalog` template and `AgentService.DefaultGraph()` set `OutputTemplate` on their `end` node for exactly this reason — forgetting it on a new template/graph produces a silently empty reply, not an error.
...
The most common way to trip this: adding a new `WorkflowNode` subclass without adding its case to `WorkflowNodeConverter.Read` (`AgentStudio.Domain/AgentStudioJson.cs`) — the resulting `JsonException` during graph deserialization used to land outside the try/finally. **Whenever you add a new node type, add its JSON converter case and its `GraphMapper.ToDomain`/`ToDto` case in the same change**, before writing anything else. `AgentStudio.Tests/NodeTypeRoundTripTests.cs` reflects over every `WorkflowNode` subclass and round-trips an instance through both places — a forgotten `WorkflowNodeConverter`/`GraphMapper.ToDomain` case fails the test loudly (instead of surfacing as the deadlock above); a forgotten `GraphMapper.ToDto` case is the sneakier one (its `switch` has no `default`, so a missing case silently serializes an empty `Props` dict instead of throwing) — caught by asserting any node type with real properties of its own comes back with a non-empty `Props`. This is deliberately a test, not a node-type registry: a registry can't cover `GraphMapper`'s hand-written `Props` mapping without a form-metadata engine, which is a bigger abstraction than the actual problem justifies.
...
**Node execution is Strategy-pattern for the 9 "leaf" node types** (`http`, `documentSearch`, `databaseQuery`, `jsonParse`, `expression`, `collectionGet`/`Set`, `integrator`, `prompt` — every type whose next edge is always the single unconditional `NextByEdge(graph, id, branch: null)`, i.e. `RunStepAsync`'s list minus `subAgent`). Each is its own `INodeExecutor` implementation in `AgentStudio.Application/NodeExecutors.cs` (`HttpNodeExecutor`, `PromptNodeExecutor`, ...), dispatched by `WorkflowRunner`'s `_executorsByType` dictionary (keyed by `WorkflowNode` subclass, built once in the constructor **from the runner's own existing injected services** — deliberately not DI-registered separately, so the public constructor signature (and every existing test/caller) didn't change). `start`/`end`/`condition`/`parallel`/`join`/`subAgent` stay as explicit `switch` cases in `RunSegmentAsync` — their control flow genuinely isn't uniform (branch selection, multi-branch fan-out, or — for `subAgent` — a recursive call back into `RunAsync` itself), so forcing them into the same "just returns a detail string" executor shape would need the interface to grow escape hatches that defeat the abstraction. Adding a new leaf node type: implement `NodeExecutor<TNode>`, add one line to the `_executorsByType` list in `WorkflowRunner`'s constructor — no `switch` case, no `RunStepAsync` boilerplate.
...
- `ParallelNode`/`JoinNode` fan out branches concurrently, each running against its own variable snapshot merged back (later-declared branch wins a key conflict) once all branches finish. The per-branch `ChunkSink` buffer-and-flush-in-declared-order machinery still exists (in case a future node type ever emits live again) but is inert today — no leaf node populates it, so every flush is a no-op.
