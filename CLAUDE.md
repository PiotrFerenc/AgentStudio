...
### Human-in-the-loop approval (phase 13)

`ApprovalNode { Message, ResultVariable }` suspends a run — reaching it in `WorkflowRunner` persists a `PendingApproval` (`AgentId`, `AgentVersion`, `NodeId`, `Message`, a full `Variables` snapshot, `Status`) and stops (`current = null`), the same way reaching an `EndNode` with no further edges stops, not an error. Requires exactly two outgoing edges, `Branch` "approved"/"rejected" — `WorkflowValidator` enforces both are present, same check shape as `ConditionNode`'s true/false. The `/approvals` page (any authenticated user, not Admin-only) lists pending ones and lets someone Approve/Reject. `WorkflowRunner.ResumeApprovalAsync(pendingApprovalId, approved, decidedBy)` resolves `NextByEdge(graph, node.Id, approved ? "approved" : "rejected")` and continues from there via the same private `RunSegmentAsync` every other entry point uses, but as a **genuinely independent execution** — fresh `ConversationState` (`resume-{guid}` prefix), fresh `ExecutionLog`, restored only from the variable snapshot (no message history replay: a `PromptNode` reached after resuming sees no prior turns). Unlike `DebugNodeAsync`, this log **is** persisted (`CompleteAsync` is called) — it's a real continuation of a real run. Debugging an `ApprovalNode` via "Test this node" creates a real `PendingApproval` row (same "real calls in debug mode" policy as HTTP/DB/LLM nodes) — the debug panel's help text says so.
