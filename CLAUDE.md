...
### Model providers in appsettings.json

Providers can be defined two ways: the database (`Providers` table, admin-managed via `/providers`, the original mechanism) or `appsettings.json`'s `"ModelProviders"` array (operational config, same split as `DatabaseConnections`). `ProviderRepository.ListAsync`/`GetByNameAsync` merge both — a config provider wins on a name collision with a DB row, same "config is the deployment's source of truth" precedent as `DatabaseConnectionProvider`. Config providers get a deterministic `Id` (`DeterministicGuid.From($"provider:{name}")`, `AgentStudio.Domain/DeterministicGuid.cs` — SHA-256 of the seed string, first 16 bytes) since appsettings has nowhere to persist a random one, and `ModelProviderConfig.IsFromConfig` (`[NotMapped]`, set only by `ProviderRepository`) lets `/providers` show a Source column and skip offering edit/delete for rows it can't actually change there. `ModelProviderConfig.Headers` (`Dictionary<string,string>`, jsonb + `JsonValueComparer` like `Agent.EnvironmentVariables`) are extra HTTP headers sent with every request to that provider — e.g. a gateway auth header some OpenAI-compatible proxy expects — applied via a `PipelinePolicy` (`ProviderClientOptions.Build`, `AgentStudio.Infrastructure`) that both `OpenAiCompatibleChatClientFactory` and `OpenAiCompatibleEmbeddingClientFactory` now call instead of building `OpenAIClientOptions` inline (deduplicates what used to be copy-pasted timeout-setting code in both factories).
...
Users can also come from `appsettings.json`'s `"Users"` array — same merge-with-DB, config-wins-on-collision pattern as `ModelProviders`/`DatabaseConnections`. Each entry (`AgentStudio.Domain/User.cs`'s `UserConfig`) sets either `Password` (plaintext, dev convenience — compared directly in `UserService.VerifyPasswordAsync` via `CryptographicOperations.FixedTimeEquals`, never hashed or stored) or `PasswordHash` (a real `PasswordHasher<User>` hash, for production). `UserRepository` gives config users a deterministic `Id` (`DeterministicGuid.From($"user:{username}")`) — unlike providers, this one actually matters at runtime: `Agent.OwnerId`/`AgentCollaborator.UserId` are FKs into user Ids, so a config account's Id has to stay stable across restarts for `AgentAccess.CanEdit` to keep recognizing it as the same owner/collaborator. `User.IsFromConfig`/`ConfigPlaintextPassword` are `[NotMapped]`, same pattern as `ModelProviderConfig.IsFromConfig`. `UserService.ChangeRoleAsync`/`DeleteAsync` reject a config-sourced user outright (edit `appsettings.json` and restart instead) — checked before the "last admin" guard, so a config Admin doesn't block a DB Admin from being demoted/deleted and vice versa via that separate check. `AnyUsersAsync` (gates the `/setup` bootstrap redirect) counts config users too, so a deployment with only a config-defined Admin never sees the bootstrap flow.
...
`start`, `prompt`, `message`, `condition`, `http`, `variable`, `documentSearch`, `subAgent`, `databaseQuery`, `jsonParse`, `expression`, `collectionGet`, `collectionSet`, `approval`, `integrator`, `parallel`, `join`, `end`. Placeholders in templates: `{input}`, `{variables.name}`. Condition operators: `Equals`, `NotEquals`, `Contains`, `StartsWith`, `EndsWith`, `>`, `<`, `>=`, `<=`.
